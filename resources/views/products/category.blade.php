...
<x-meta-tags :metaData="[
    ...
    'otherMetaTags' => $category->other_meta_tags ?? '',
]" />
@push('css')
    <!-- SWIPER -->
...
@section('content')
    <main id="pageWrapper" class="productPage">
        <section id="ProductHero">
            <img src="{{ $category->banner_value }}" width="1920" height="1080" loading="lazy"
                alt="{{ $category->banner_alt_text_value }}" class="dsk">
            <img src="{{ $category->banner_mobile_value }}" alt="{{ $category->banner_alt_text_value }}" class="mob">
        </section>
                                ...
                                <li>
                                    <label class="filterBxWrap">
                                        <input name="category[]" class="categoryFilter" type="radio"
                                            ...
                                            {{ $categoryFilter->id == $category->id ? 'checked' : '' }}>
                                        <div class="filterBx">
                                            <div class="cntWrap">
    ...
    <script>
        ...
        $(document).on('click', '.categoryFilter, #bestsellerToggle', function() {
            ...
            if (categoryName !== undefined) {
                window.history.pushState({
                    ...
                    `/product-category/${categoryName}` // new URL
                );
            }
        ...
        function loadBannerAndMeta() {
            $.ajax({
                ...
                success: function(response) {
                    if (response.status) {
                        $('#ProductHero img.dsk').attr('src', response.data.banner_value);
                        $('#ProductHero img.mob').attr('src', response.data.banner_mobile_value);
