...
use Modules\Admin\Http\Requests\SolutionRequest;
...
class SolutionController extends Controller
{
            ...
            return DataTables::eloquent($data)
                ...
                ->editColumn('sort_order', function ($row) {
                    return '<input type="text" value="' . $row->sort_order . '"
                        class="form-control w-50 sort-order numeric-input" data-model="Solution"
                ...
                ->editColumn('status', function ($row) {
                    ...
                    return '<div class="form-check form-switch">
                        <input type="checkbox" id="status' . base64_encode($row->id) . '" class="form-check-input toggle-switch" data-name="Status" data-labels="Enabled;Disabled" data-column="status" data-model="Solution"
                            value="' . $row->status . '"
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<a href="' . route('solutions.edit', base64_encode($row->id)) . '" class="btn btn-primary btn-sm mr-1" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Edit" style="margin-right: 3px;"><i class="fas fa-edit"></i></a>';
                    $btn .= '<form action="' . route('solutions.destroy', base64_encode($row->id))  . '" method="POST" style="display: inline-block;">' . csrf_field() . method_field('DELETE') . '<button type="button" class="btn btn-danger btn-sm delete-btn" data-delete-message-type="itemOnly" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Delete"><i class="fas fa-trash"></i></button></form>';
                    return $btn;
                })
    ...
    {
        $sort_order = Solution::max('sort_order') + 1;
        return view('admin::solution.form', compact('sort_order'));
    }
    ...
    {
        $solution = Solution::findOrFail(base64_decode($id));
        return view('admin::solution.form', compact('solution'));
    }
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            return response()->json(['success' => false, 'message' => 'The requested Solution does not exist.']);
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error($e);
