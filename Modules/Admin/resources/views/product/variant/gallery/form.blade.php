...
@section('title', isset($productGallery) ? 'Edit Product Variant Gallery' : 'Create Product Variant Gallery')
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">
        <a href="{{ route('product-variants.galleries.index', base64_encode($productVariant->id)) }}">
...
@section('content')
    <!-- form start -->
    <form method="POST"
        action="{{ isset($productGallery) ? route('product-variants.galleries.update', ['product_variant' => base64_encode($productVariant->id), 'gallery' => base64_encode($productGallery->id)]) : route('product-variants.galleries.store', base64_encode($productVariant->id)) }}"
        ...
        @endisset
        <div class="row">
            <x-admin::action-buttons :cancel-url="route('product-variants.galleries.index', base64_encode($productVariant->id))" save-label="{{ isset($productGallery) ? 'Update' : 'Create' }}" />
            <div class="col-md-12">
                <div class="card">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('form').customValidate({
                ...
                successRoute: "{{ route('product-variants.galleries.index', base64_encode($productVariant->id)) }}"
            });
        });
    </script>
@endpush
