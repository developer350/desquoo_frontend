...
@push('js')
    ...
    <script>
        ...
        // Sanitizes, truncates, and formats plain text or HTML content for safe display in DataTables.
        function formatData(data, maxLength = 30, textSelector = 'span') {
            if (!data) return '';

            const looksLikeHtml = /<[^>]+>/.test(data);

            if (!looksLikeHtml) {
                // ---- Plain text path ----
                const sanitized = $.sanitize(data);
                const isLong = sanitized.length > maxLength;
                const short = isLong ? sanitized.slice(0, maxLength) + '...' : sanitized;
                const title = isLong ? ` title="${sanitized}"` : '';
                return `<span${title}>${short}</span>`;
            }
            ...
            const $tmp = $('<div>').html(cleanHtml);

            // Find the text element (default: <span> inside product cell)
            const $textEl = $tmp.find(textSelector).first();
            const fullText = $.sanitize($textEl.text() || '');
            const isLong = fullText.length > maxLength;
            const short = isLong ? fullText.slice(0, maxLength) + '...' : fullText;

            if ($textEl.length) {
                $textEl.text(short);
                if (isLong) $textEl.attr('title', fullText);
            }
